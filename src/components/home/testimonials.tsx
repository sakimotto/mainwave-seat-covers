  ...
  return (
    <section className="bg-ink-soft py-20 md:py-28 border-y border-white/5">
      <div className="container-wide">
        <Reveal>
          <p className="text-mainwave-red text-xs font-bold tracking-[0.4em] uppercase mb-4">{dict.testimonials.kicker}</p>
          <h2 className="text-display text-[clamp(2.2rem,5.5vw,4.5rem)] text-bone mb-12">
            {dict.testimonials.title1}<br />{dict.testimonials.title2}
          </h2>
        </Reveal>
          ...
          {dict.testimonials.items.map((t, i) => (
            <Reveal key={t.name} delay={i * 120}>
              <figure className="h-full border border-white/10 bg-ink p-6 md:p-8 flex flex-col">
                <div className="flex gap-1 mb-5" aria-label={dict.testimonials.starsLabel}>
